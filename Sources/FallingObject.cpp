...
void FallingObject::update(float catX, float catY, bool activate)
{
	...
	if (position.y() + width > floorHeight || center.y() + radius >= floorHeight)
...
void FallingObject::render(Kore::Graphics2::Graphics2* g2, float cameraX, float cameraY, float width, float height)
{
		...
		&& cameraY < position.y() && cameraY + height > position.y() + this->height)
	{
		...
		float currentRotation = (frameCount / maxFrameCount) * maxRotation;
		g2->pushRotation(currentRotation, center.x()-cameraX, center.y());
		g2->drawScaledSubImage(tex, 0, 0, this->width, this->height, position.x() - cameraX, position.y() , this->width, this->height);
